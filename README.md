**BookingRoom** est une plateforme web de réservation de salles d'événements avec authentification utilisateur, gestion de catalogue de salles et panel d'administration complet.

##  Schéma Base de Données
src/main/resources/database/schema.sql    ← ICI

### 🔐 Connexion
![image alt]([./screenshots/01_login.png](https://github.com/bassem-ch007/room_booking/blob/main/Capture%20d'%C3%A9cran%202025-12-17%20235649.png?raw=true))

### 🏢 Catalogue des salles
![Rooms List](./screenshots/02_rooms_list.png)

### 📝 Détail d'une salle
![Room Details](./screenshots/03_room_details.png)

### 📅 Créer une réservation
![Create Reservation](./screenshots/04_create_reservation.png)

### 📋 Mes réservations
![My Reservations](./screenshots/05_my_reservations.png)

### 🛠️ Dashboard Admin
![Admin Dashboard](./screenshots/06_admin_dashboard.png)

### 👥 Gestion des utilisateurs
![Admin Users](./screenshots/07_admin_users.png)

### 🏢 Gestion des salles
![Admin Rooms](./screenshots/08_admin_rooms.png)

### 📊 Gestion des réservations
![Admin Reservations](./screenshots/09_admin_reservations.png)

### 👤 Profil utilisateur
![Profile](./screenshots/10_profile.png

